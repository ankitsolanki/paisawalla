...
File: `server/api/services/lenderRuleEngine.js`
...
File: `server/api/controllers/eligibilityController.js`
...
### Inject Scripts (in `client/src/embed/` and `client/src/embed/`)
...
File: `server/api/seeds/seedLenderData.js`
...
File: `server/api/utils/curlLogger.js`
...
File: `server/api/services/karixOtpService.js`
Controller: `server/api/controllers/authController.js`
...
**Fix**: Changed to `response.data?.applicationId` in both `client/src/components/EligibilityChecking.jsx` and `client/src/components/EligibilityChecking.jsx`.
