...
app.use('/api/ecv', ecvRoutes);
app.use('/api/lender-eligibility', lenderEligibilityRoutes);
app.use('/api/eligibility', eligibilityRoutes);
app.use('/api/admin/api-logs', apiLogRoutes);

app.get('/api/test-variants', (req, res) => {
  res.status(200).json({ ok: true, variants: VARIANT_INFO });
});

app.post('/api/test-variants/seed', async (req, res) => {
  try {
